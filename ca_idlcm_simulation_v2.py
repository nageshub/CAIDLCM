...
def run_ca_idlcm_simulation(num_vehicles=30, num_rsus=5, total_steps=1200, seed=42, algorithm="CA-IDLCM"):
    """Improved simulation with better algorithm differentiation"""
    random.seed(seed)
    np.random.seed(seed)
    ...
    for t in tqdm(range(total_steps), desc=f"Simulation ({algorithm}, seed={seed})"):
        ...
        for node in nodes:
            ...
            if random.random() < lambda_t:
                ...
                # === STRONGER DIFFERENTIATION BETWEEN ALGORITHMS ===
                if algorithm == "CA-IDLCM":
                    action = agents[node.node_id].select_action(context)
                    if action == "ScoreBased":
                        tips = score_based_tip_selection(node.local_tangle, originator_connectivity=node.connectivity)
                    elif action.startswith("URTS"):
                        tips = urts_tip_selection(node.local_tangle, k=3 if "k3" in action else 2)
                    else:
                ...
                if hasattr(node.local_tangle, 'tips') and len(node.local_tangle.tips) > 12:
                    ...
                    node.local_tangle.tips = set(valid_tips[-8:])
        ...
        if t % 100 == 0:
            ...
            orphans = sum(1 for tx_id, node_obj in all_transactions.items() 
                         if len(list(node_obj.local_tangle.graph.successors(tx_id))) == 0 and (t - node_obj.local_tangle.graph.nodes[tx_id].get('timestamp', 0) > 150))
            confirmed_count = sum(1 for tx_id, node_obj in all_transactions.items() 
                                if len(list(node_obj.local_tangle.graph.successors(tx_id))) > 0)
            
            stats['orphan_fraction'].append(orphans / max(total_txs, 1))
            stats['confirmation_rate'].append(confirmed_count / max(total_txs, 1))
